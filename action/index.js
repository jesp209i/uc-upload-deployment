...
async function uploadDeployment(callUrl, apiKey, filePath) {
  const file = (0, import_fs.createReadStream)(filePath);
  const headers = {
    "Content-Type": "multipart/form-data",
    "Umbraco-Api-Key": apiKey
  };
  const uploadResponse = await axios_default.post(callUrl, {
    file
  }, {
    headers
...
async function run() {
  try {
    ...
    const deploymentId = (0, import_core2.getInput)("deployment-id");
    const filePath = (0, import_core2.getInput)("file-path");
    (0, import_core2.info)(filePath);
    ...
    (0, import_core2.info)("Source Package uploaded successfully");
  } catch (error) {
    (0, import_core2.warning)(`Error: ${error}`);
    (0, import_core2.setFailed)("Got an error while trying to upload source deployment package");
  }
}
run();
